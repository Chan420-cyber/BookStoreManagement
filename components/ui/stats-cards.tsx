  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${className}`}>
      {stats.map((stat, index) => (
        <div 
          key={index} 
          className={`relative bg-gradient-to-br ${stat.gradient || 'from-gray-800/50 to-gray-800/30'} bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-lg p-6 border ${stat.borderColor || 'border-gray-700/50'} transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl group cursor-pointer overflow-hidden`}
        >
          <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-r ${stat.gradient || ''}`} />
          
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div className={`${stat.color} p-3 rounded-xl shadow-lg transition-all duration-300 group-hover:scale-110`}>
